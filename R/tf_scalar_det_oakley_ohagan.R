...
#' Dimensions 15
#'
#' @param x Inputs of dimension (at least) 15. See below for details.
...
oo15 <- function(x, scale01=TRUE){
  if(scale01){
    RR <- cbind(rep(-3, 15), rep(3, 15))
    x[1:15] <- x[1:15]*(RR[,2] - RR[,1]) + RR[,1]
  }
  xx <- matrix(x[1:15], 15, 1)
